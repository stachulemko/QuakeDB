...
std::vector<int64_t> Table::getBlokcNum() {
...
void Table::addBtree(std::string columnName) {
    for (int i = 0; i < dataBlocks.size(); i++) {
...
std::vector<int> Table::getBlockNum(std::string columnName, allVars data) {
    for (int i = 0; i < dataBlocks.size(); i++) {
        if (dataBlocks[i].ifColumnExists(columnName)) {
...
std::vector<std::vector<allVars>>Table::getRowsByBtree(std::string columnName, allVars data) {
...
bool Table::ifBtreeColumnExists(std::string columName) {
    int index = 0;
    for (int i = 0; i < dataBlocks.size(); i++) {
