...
public:
	...
	std::vector<uint8_t> MarshalColumn() {
		...
		std::vector<uint8_t>* lenghtColumn;
		lenghtColumn = marshalInt32_t(*columnSize);
		result.insert(result.end(), columnTypeBytes->begin(), columnTypeBytes->end());
		result.insert(result.end(), lenghtColumn->begin(), lenghtColumn->end());
		...
		delete columnTypeBytes, columnTypeBytes;
		return result;
	}
	void loadloadAllBytesToDecode(std::vector<uint8_t>bytes) {
		clearAll();
    ...
    void showColumn() {
        ...
        // Wyœwietl typ kolumny (ID)
        std::cout << "Column Type ID: " << columnTypeId << std::endl;

        // Wyœwietl rozmiar kolumny
        ...
        // Wyœwietl nazwê kolumny
        std::cout << "Name: " << getColumnName() << std::endl;

        // Wyœwietl typ danych kolumny
        std::cout << "Data Type: " << getColumnType() << std::endl;

        // Wyœwietl czy kolumna dopuszcza wartoœci NULL
